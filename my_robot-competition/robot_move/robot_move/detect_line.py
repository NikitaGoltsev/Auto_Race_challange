...
class DetectLane(Node):
	...
	def __init__(self):
		...
		self.publisher_ = self.create_publisher(Float64, '/detect/line', 20)
		self.subscription = self.create_subscription(Image, '/color/image_projected_compensated', self.cbFindLane, 20)
		#self.subscription = self.create_subscription(Image, '/color/image', self.image_save, 1)
	...
	def image_save(self, image_msg):
		cv_image = self.br.imgmsg_to_cv2(image_msg, "bgr8")
		if self.index % 5 ==0:
			path = f'/home/sear/yolo/img_for_yolo{int(self.index/5)}.jpg'
			cv2.imwrite(path, cv_image)
			self.get_logger().info('Message data: %s' % path )
	...
	def make_lane(self, cv_image, white_fraction, yellow_fraction):
		...
		if self.is_center_x_exist == True:
			...
			msg_desired_center.data = centerx.item(405)#350
